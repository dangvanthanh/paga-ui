...
const api = computed(() =>
  zagSelect.connect(state.value, send, normalizeProps)
);
</script>
  ...
  <Teleport to="body">
    <div v-bind="api.positionerProps">
      <ul v-bind="api.contentProps" :class="selectClasses.content">
        <li v-for="item in selectData" :key="item.value" v-bind="api.getItemProps({ item })" :class="selectClasses.item">
          <span :class="selectClasses.itemLabel">{{ item.label }}</span>
          <span v-bind="api.getItemIndicatorProps({ item })" :class="selectClasses.itemIndicator">✓</span>
        </li>
      </ul>
    </div>
  </Teleport>
