...
const service = useMachine(zagTimer.machine, {
	id: useId(),
	countdown: true,
	autoStart: false,
	startMs: zagTimer.parse({ days: 1, seconds: 0 }),
})
const api = computed(() => zagTimer.connect(service, normalizeProps))
</script>
	...
	<div v-bind="api.getRootProps()" :class="styles.root">
		<div v-bind="api.getAreaProps()" :class="styles.area">
			<div v-bind="api.getItemProps({ type: 'days' })" :class="styles.item">
				<span :class="css({ fontSize: '2xl' })"
					>{{ api.formattedTime.days }}</span
				>
			...
			<div v-bind="api.getItemProps({ type: 'minutes' })" :class="styles.item">
				<span :class="css({ fontSize: '2xl' })">
					{{ api.formattedTime.minutes }}</span
				>
			...
			<div v-bind="api.getItemProps({ type: 'seconds' })" :class="styles.item">
				<span :class="css({ fontSize: '2xl' })">
					{{ api.formattedTime.seconds }}</span
				>
