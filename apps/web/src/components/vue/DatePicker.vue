...
const service = useMachine(zagDatepicker.machine, {
	id: useId(),
	view: 'day',
})
const api = computed(() => zagDatepicker.connect(service, normalizeProps))
</script>

<template>
	<div :class="flex({ direction: 'column', gap: 1.5 })">
		<label v-bind="api.getLabelProps()" :class="css({ textStyle: 'sm' })">
			Date Picker
		</label>
		<div
